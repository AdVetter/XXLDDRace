...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	if(((CServer *) Server())->m_aPrevStates[ClientID] < CServer::CClient::STATE_INGAME)
	{
		...
		SendChatTarget(ClientID, "   XXLDDrace64 "  XXL_VERSION " by XXLTomate");
...
//		Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "server", aBuf);
	...
	if(MsgID == NETMSGTYPE_CL_SAY)
	{
		...
		if(pMsg->m_pMessage[0]=='/')
		{
			...
			if (pPlayer->m_Authed)
				Console()->SetAccessLevel(pPlayer->m_Authed == CServer::AUTHED_ADMIN ? IConsole::ACCESS_LEVEL_ADMIN : pPlayer->m_Authed == CServer::AUTHED_SUBADMIN ? IConsole::ACCESS_LEVEL_SUBADMIN : pPlayer->m_Authed == CServer::AUTHED_MOD ? IConsole::ACCESS_LEVEL_MOD : IConsole::ACCESS_LEVEL_HELPER);
			else
		...
		else
		{
			...
			else if(!str_comp_nocase(pMsg->m_pMessage, "help") && g_Config.m_SvHelper)
			{
				...
				Console()->ExecuteLine("helper", ClientID);
			}
			else
				SendChat(ClientID, Team, pMsg->m_pMessage, ClientID);
		}
	...
	else if (MsgID == NETMSGTYPE_CL_EMOTICON && !m_World.m_Paused)
	{
		...
		if(g_Config.m_SvSpamprotection && pPlayer->m_LastEmote && pPlayer->m_LastEmote + ((float)g_Config.m_SvEmoticonDelay / 1000) * Server()->TickSpeed() > Server()->Tick())
			return;
